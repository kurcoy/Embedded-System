...
int8_t FLASH_If_Erase(uint32_t StartSector)
{
  ...
  {
	/* the first 128KB is used for boot loader, the remaining is used for application */
...
uint32_t FLASH_If_Write(__IO uint32_t* FlashAddress, uint32_t* Data ,uint16_t DataLength)
{
  ...
  for (i = 0; (i < DataLength) && (*FlashAddress <= (USER_FLASH_END_ADDRESS-4)); i++)
  {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
	  ...
	  switch(*FlashAddress)
	  {
		case FLASH_SECTOR0_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_0, VoltageRange_3);
		  break;
		case FLASH_SECTOR1_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_1, VoltageRange_3);
		  break;
		case FLASH_SECTOR2_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_2, VoltageRange_3);
		  break;
		case FLASH_SECTOR3_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_3, VoltageRange_3);
		  break;
		case FLASH_SECTOR4_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_4, VoltageRange_3);
		  break;
		case FLASH_SECTOR5_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_5, VoltageRange_3);
		  break;
		case FLASH_SECTOR6_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_6, VoltageRange_3);
		  break;
		case FLASH_SECTOR7_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_7, VoltageRange_3);
		  break;
		case FLASH_SECTOR8_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_8, VoltageRange_3);
		  break;
		case FLASH_SECTOR9_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_9, VoltageRange_3);
		  break;
		case FLASH_SECTOR10_START_ADD:
		  FLASH_EraseSector(FLASH_Sector_10, VoltageRange_3);
		  break;
		case FLASH_SECTOR11_START_ADD:
